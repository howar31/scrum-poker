...
- **ICE Servers**: STUN is always present (`stun.l.google.com:19302`, `global.stun.twilio.com:3478`). TURN is opt-in via build-time env vars in `peerManager.ts`'s `buildIceServers()`: `VITE_TURN_OPENRELAY_URLS` / `_USERNAME` / `_CREDENTIAL` for Open Relay (Metered.ca, 20 GB/mo free, public creds OK in bundle) and `VITE_TURN_CF_TOKEN_URL` for Cloudflare TURN (1 TB/mo free; Worker at `infra/turn-token-worker/` mints short-lived credentials on demand so the Cloudflare API key never reaches the client). Both providers can run simultaneously — ICE picks whichever relay candidate wins first. Cloudflare token fetch has a 2 s hard timeout and degrades silently to STUN + Open Relay on failure; TURN is never on the join critical path. The default OSS bundle ships zero credentials and behaves identically to the historical STUN-only configuration. See `.env.example` and the README's "TURN configuration" section.
...
- **`humanizePeerError(err)` helper** (`peerManager.ts`): every `peer.on('error')` routes through this before the error reaches the UI. It maps the PeerJS `err.type` to an i18n-translated friendly message AND a `toast: boolean` flag. Internal-only types (`unavailable-id` during ELECTING, `peer-unavailable` during `joinRoomWithRetry`, `disconnected` from call-order mistakes) return `toast: false` so they land in `console.error` only — no spurious popups for races the state machine already handles. Visible types (`network`, `server-error`, `socket-error`, `socket-closed`, `ssl-unavailable`, `browser-incompatible`, `webrtc`) get translated strings. Raw PeerJS English NEVER reaches the UI: both the post-init toast path and the pre-init `reject()` path substitute the humanised message (with `err.type` preserved so `isUnavailableId` / `isPeerUnavailable` callers still branch correctly on type). This also fixes the "Lost connection to server" message PeerJS emits — it would otherwise clash with the Serverless brand promise.
...
- **Chrome content column**: header, `<main>`, and `<footer>` each constrain their inner content to `max-w-6xl mx-auto` so their left and right edges line up on wide screens. Outer `<header>` / `<footer>` elements stay viewport-wide so sticky backgrounds and top/bottom borders still span edge-to-edge; only the flex row inside is boxed.
...
- **`network-flap`** — host + 2 clients; one client toggles Puppeteer offline mode for 4 s then comes back. Exercises the `online` / `offline` window listeners in `peerManager.attachNetworkListeners()`. Asserts the client stays in the room, no host migration is triggered (epoch stable), and the store's `connectionStatus` visibly passes through a `reconnecting-*` state before returning to `connected`. (Limitation: Puppeteer offline mode blocks HTTP / WebSocket but not already-open UDP / WebRTC sockets, so this primarily verifies the listener wiring, not real ICE recovery.) **Exits?** SIGINT. **Asserts:** `stayedInRoom=true noMigration=true playerCountStable=true recovered=true sawReconnecting=true`.
- **`ice-restart`** — host + 2 clients; one client calls the VITE_E2E-gated `window.__POKER_PEER__.restartIce()` hook. Exercises `restartIceAllConns()` directly without any simulated failure. Asserts DataConnection survives, no migration occurs, and the store transitions through `reconnecting-ice` back to `connected`. **Exits?** SIGINT. **Asserts:** `hookFired=true stayedInRoom=true noMigration=true playerCountStable=true recovered=true sawIceReconnecting=true`.
...
- **Arc Browser as Host**: Arc's WebRTC stack (even with `arc://flags` → "Anonymize local IPs exposed to WebRTC" Disabled) intermittently fails ICE with Chrome peers, affecting both initial joins via `?room=XYZ` AND post-migration reconnects. `ArcBrowserWarning` advises Arc users to decline Host. `handleIncomingConnection` caps half-open RTCPeerConnections via `INCOMING_OPEN_TIMEOUT_MS = 20 s` (hygiene only — underlying gap needs TURN).
