...
- **Direct-connect to successor (the ≈10 s path)**: Clients do NOT wait for the new host to reclaim the well-known ID. Instead they call `joinViaHost(successor.peerId)` using the `peerId` stored in the players map (broadcast with every STATE_UPDATE). Retries with `DIRECT_CONNECT_DELAYS_MS = [0, 0.5, 1.5, 3, 5] s` and a 3 s per-attempt timeout. This works because the successor's peer is already alive — they don't destroy it when they flip `isHost = true`.
...
- **New joiner via `?room=XYZ` during migration**: `joinRoomWithRetry(roomId, { maxDurationMs, onProgress })` in `peerManager` returns a `{ cancel, promise }` handle. Internally loops `joinViaHost(well-known)` with a 5 s per-attempt timeout until success, cancel, or window exceeded. `onProgress` reports `{ kind: 'connecting' | 'peer-unavailable' | 'timeout' }` so the UI can tell the user *why* it's still waiting: `peer-unavailable` (ID not registered — room doesn't exist, or broker just released it mid-migration) vs `timeout` (broker still has the ID but the peer is unreachable — migration likely in progress). `Home.tsx`'s `JoinForm` drives the retry state machine: 30 s initial window; on expiry shows a "Still haven't reached the room" prompt with "Keep trying (30s)" / "Give up" so the total wait can span the worst-case broker hold. Cancel is always one click away; canceling calls `handle.cancel()` which destroys any in-flight peer and rejects the promise.
...
- Home: `home-name` (input), `home-room-id` (input), `home-create`, `home-join`, `home-not-this-room`, `home-join-fallback-toggle`, `home-join-progress` (+ `data-progress`), `home-join-cancel`, `home-join-prompt`, `home-join-keep-trying`, `home-join-give-up`
