...
- Tracks `connectionStatus: 'connected' | 'reconnecting' | 'disconnected'` for the header dot. `connected` = PeerJS signaling registered and P2P live. `reconnecting` = broker WebSocket dropped / migration in progress / scheduleReconnect retrying — existing DataConnections may still flow. `disconnected` = all recovery paths exhausted, user should refresh or create a new room.
...
- **Broker re-registration**: `peer.on('disconnected')` fires when the WebSocket to PeerJS signaling drops (tab backgrounded, network blip, heartbeat timeout) while the Peer object still exists locally. Without handling, the user keeps appearing "online" to themselves but is invisible to new joiners (`peer-unavailable`). The handler flips `connectionStatus='reconnecting'` and calls PeerJS's built-in `peer.reconnect()` to re-register the same peer ID; a subsequent `peer.on('open')` flips the status back to `connected`. The handler short-circuits when `intentionalLeave` is set, and `destroy()` calls `peer.removeAllListeners()` before `peer.destroy()` — both prevent a reconnect-race during migration teardown that would hold the old peer ID alive on the broker and block the successor from reclaiming it.
...
- **Manual transfer**: The current host may transfer host privileges to another player via `TRANSFER_HOST` action. The old implementation used the new host's random peerId and never set `isHost=true` on the recipient, so incoming connections were rejected and the room looked broken. Now `transferHost` is a thin wrapper over the `HOST_LEAVING` flow: the old host broadcasts `HOST_LEAVING { nextHostId }`, **sets `intentionalLeave=true` so `peer.on('disconnected')` skips the broker reconnect during teardown**, destroys its peer (releasing the well-known ID), then rejoins as a client via `joinRoom(roomId)` with `RECLAIM_DELAYS_MS` backoff. The designated successor runs `reclaimHostIdentity` and everyone else reconnects to them at the well-known ID.
...
- **Players pill + panel** (`src/components/PlayersPanel.tsx`) — a `👥 Players N` pill sits above the Table. Clicking it opens an on-demand drawer: right-side slide-in on desktop (`md:w-96`), bottom sheet on mobile (`max-h-[75vh]`, rounded top). The panel lists every player with host crown, "You" badge, and vote status (empty circle = unvoted, green check = voted pre-reveal, mono value = post-reveal). Host-only **Make host** / **Kick** buttons next to each other player use a two-step confirmation: first click arms the button (blue/red fill + "再點一次確認" hint) and auto-disarms after 3 s; second click executes. Clicking a different player/action disarms the previous one. Backdrop click, `Escape`, or the X closes the drawer. Hover-only moderator controls on the Table are gone — the drawer is the only surface for these actions.
...
- `src/utils/`
  - `peerManager.ts` — PeerJS wrapper: `createRoom`, `joinRoom`, `scheduleReconnect`, `handleHostDisconnect`, `handleHostLeaving`, `reclaimHostIdentity`, `transferHost`, `leave`.
