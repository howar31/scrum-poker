...
- **Manual transfer**: The current host may transfer host privileges to another player via `TRANSFER_HOST` action. The old implementation used the new host's random peerId and never set `isHost=true` on the recipient, so incoming connections were rejected and the room looked broken. Now `transferHost` is a thin wrapper over the `HOST_LEAVING` flow: the old host broadcasts `HOST_LEAVING { nextHostId }`, destroys its peer (releasing the well-known ID), then rejoins as a client via `joinRoom(roomId)` with `RECLAIM_DELAYS_MS` backoff. The designated successor runs `reclaimHostIdentity` and everyone else reconnects to them at the well-known ID.
