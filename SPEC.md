...
- **App header** (`src/App.tsx`) — single horizontal bar that doubles as the room header when `roomId` is set. Left side: Scrum Poker logo, Room ID button (click = copy Room ID), link icon button (click = copy invite URL), and a small connection dot (green/red with pulse when disconnected). Right side: a Leave button (room mode only) and a `MoreVertical` trigger that opens a dropdown menu containing Language / Animations / Theme rows. The dropdown closes on outside click or `Escape`. Collapsing the three preference toggles into a single menu keeps the header compact on mobile where the right-side buttons used to overlap the Room ID.
