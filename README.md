...
- **Resilient Joins**: If a new joiner arrives while the room is switching hosts, the Join form turns into a cancellable retry with a spinner and a progress message that distinguishes "can't find this room yet" from "host isn't responding". Defaults to a 30-second window, then offers "Keep trying (30 s) / Give up" so you can outlast the worst-case broker hold without ever being stuck on a silent spinner.
- **Graceful Leave / Transfer**: When the host clicks Leave or transfers host, the leaving host broadcasts the authoritative successor to every client before tearing down, skipping the probe step entirely. A dedicated overlay tells users the room is switching so the app never looks frozen. After a migration, a 20 s sweep clears any players who didn't make it back (e.g., the crashed old host) so the player list stays accurate.
