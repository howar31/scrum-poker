...
- **Resilient Connections**: Automatic reconnect with exponential backoff (5 attempts over ~31 s) for unplanned disconnects. When the host clicks Leave, a graceful handoff skips the grace period — the leaving host designates a successor, who reclaims the well-known peer ID so remaining clients reconnect in ~1–2 seconds instead of 30. A dedicated overlay tells users the room is reclaiming / waiting so the app never looks frozen.
