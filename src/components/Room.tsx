          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 lg:gap-6 justify-items-center bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            {CARDS.map((card) => (
              <button
                ...
                disabled={isRevealed}
                className={cn(
                  "relative w-full aspect-[2.5/3.5] max-w-[100px] rounded-xl border-2 transition-all duration-200 transform hover:-translate-y-2 hover:shadow-lg focus:outline-none",
                  myPlayer?.card === card 
                    ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 scale-105 shadow-md"
            ...
            <div className="flex-1 overflow-y-auto p-2">
              <div className="flex flex-col gap-2">
                {allPlayers.map((player) => (
                  <div key={player.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-750 group">
